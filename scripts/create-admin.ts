 ...
 *
 * 可选参数:
 *   --email <email>    指定邮箱（默认: admin@kyads.com）
 *   --password <pwd>   指定密码（默认: Admin123）
 *   --name <name>      指定名称（默认: 管理员）
 *   --reset            强制重置已存在的管理员
 *
 * 示例:
 *   npx ts-node --compiler-options '{"module":"commonjs"}' scripts/create-admin.ts --email admin@example.com --password MySecurePass123
 */
