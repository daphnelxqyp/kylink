/**
 * CreatorFlare Monetization API 测试脚本
 * 
 * 功能：获取 CreatorFlare 联盟的所有商家 tracking_url 和相关数据
 * 
 * 使用方法：
 *   npx ts-node scripts/test-creatorflare-api.ts
 * 
 * 或者设置环境变量后运行：
 *   CF_TOKEN=your_token npx ts-node scripts/test-creatorflare-api.ts
 */
...
const API_URL = 'https://api.creatorflare.com/api/monetization'

// API Token（可通过环境变量覆盖）
// 示例 token 来自用户提供的文档
const DEFAULT_TOKEN = '440ad4115875ebf7c819b664c3c649bb'
const TOKEN = process.env.CF_TOKEN || DEFAULT_TOKEN
...
async function main() {
  console.log('🚀 CreatorFlare Monetization API 测试')
  console.log('='.repeat(80))
  ...
  try {
    ...
    // 导出到 JSON 文件
    await exportToJson(merchants, 'creatorflare-merchants.json')
