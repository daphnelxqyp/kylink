...
2026-02-06：链接管理模块移除"刷新列表"按钮，只保留"刷新广告系列"（从 Sheet 导入）。
2026-02-06：链接管理和库存管理列表默认每页显示 50 条。
2026-02-06：库存管理模块移除"刷新"按钮，只保留"补货所有低水位"操作。
2026-02-06：库存管理统计卡片"租约中库存"改为"已消耗库存"，表格移除"租约中"列。
2026-02-06：实现刷点击功能：链接管理列表新增"刷点击"按钮，通过代理IP模拟真人访问联盟链接，按人类作息曲线在当日内分布点击，Cron每分钟执行到期任务。
2026-02-06：修复脚本日志大面积丢失问题：合并写入日志为单行、压缩阶段1输出、静默成功回传、运行报告末尾新增"写入明细"确保完整写入记录不丢失。
2026-02-06：修复链接管理列表国家和域名显示不一致问题：在 campaigns/sync 和 assignment-service 写入时统一调用 normalizeCountryCode 和 extractRootDomain 标准化。
2026-02-06：库存管理列表排序与链接管理保持一致，按 campaignName 降序排列。
2026-02-06：设置页"更新联盟链接"按钮暂时禁用（灰色不可用）。
2026-02-07：管理员菜单移除链接管理、库存管理、设置三个员工专属模块。
2026-02-07：实现员工专属概览页面（工作台），展示今日换链次数、写入成功率、库存概览、点击概览和最近换链记录；管理员概览保持不变。
2026-02-07：压缩脚本日志输出（启动信息1行、阶段去空行、运行报告3行、删除写入明细），修复后端幂等路径未返回 availableStock 问题。
